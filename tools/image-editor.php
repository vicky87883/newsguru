...
<head>
    ...
    <style>
        ...
        .shape-controls, .text-controls, .buttons {
            display: flex;
            flex-wrap: wrap;
            margin-top: 10px;
        }

        .shape-controls .button, .text-controls .button, .buttons .button {
            margin: 5px;
            padding: 10px;
            border: none;
            border-radius: 8px;
        ...
        .shape-controls .button:hover, .text-controls .button:hover, .buttons .button:hover {
            background-color: #0056b3;
        }

        #colorPicker, #lineWidth, #textInput {
            margin: 5px;
            padding: 10px;
            border-radius: 5px;
...
<body>
    ...
    <script>
        ...
        document.getElementById('sharpen').addEventListener('input', function () {
            applyFilters(); // Reapply filters after sharpen
            ...
            if (sharpenValue > 0) {
                ...
                for (let y = 0; y < h; y++) {
                    for (let x = 0; x < w; x++) {
                        ...
                        data[dstOff] = r * sharpenValue;
                        data[dstOff + 1] = g * sharpenValue;
                        data[dstOff + 2] = b * sharpenValue;
        ...
        function drawText() {
            ctx.drawImage(img, 0, 0); // Re-draw the background image
            ctx.font = `${lineWidth * 10}px ${font}`;
            ctx.fillStyle = color;
            ctx.fillText(text, textX, textY);
        }

        document.getElementById('move-text').addEventListener('click', function () {
            moveTextMode = true;
            textMode = false;
        });
        ...
        function endCrop(e) {
            ...
            let cropWidth = Math.abs(startX - endX);
            let cropHeight = Math.abs(startY - endY);
